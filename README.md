...
3. Add this repository URL: `https://github.com/wanball/air-quality-card` and select **Dashboard** as the Type.
4. Click **Add**, then search for **Air Quality Card (Advanced)** and install it.
5. (Optional) HACS automatically downloads images, but for complete offline usage, you can copy files from the `img/` directory to `config/www/community/air-quality-card/img/`.

#### Method 2: Manual Installation
1. Download the compiled `air-quality-card.js` from the `dist/` directory of this repository.
2. Upload the file to your Home Assistant server inside the `config/www/` directory (e.g., `config/www/air-quality-card/dist/air-quality-card.js`).
3. Download the `img/` directory and upload it to the same folder so the structure matches.
4. Register the resource in your Home Assistant Dashboard configuration:
   - Go to **Settings** → **Dashboards** → Click Three Dots (Top Right) → **Resources**.
   - Click **Add Resource** and enter the following details:
     - **URL:** `/local/air-quality-card/dist/air-quality-card.js`
...
3. วางลิงก์ URL: `https://github.com/wanball/air-quality-card` และเลือกประเภท (Type) เป็น **Dashboard** (หรือ **Lovelace**)
4. กด **Add** จากนั้นค้นหาชื่อโปรเจกต์ **Air Quality Card (Advanced)** เพื่อสั่งติดตั้ง
5. *(ข้อมูลเพิ่มเติม)* ระบบ HACS จะดาวน์โหลดการ์ดและรูปภาพให้อัตโนมัติและมีระบบดึงภาพสำรองออนไลน์ แต่หากต้องการใช้งานแบบออฟไลน์ 100% คุณสามารถคัดลอกไฟล์รูปภาพแสดงสถานะทั้งหมดจากโฟลเดอร์ `img` ในโปรเจกต์นี้ ไปวางไว้ที่พาธ: `config/www/community/air-quality-card/img/`

#### วิธีที่ 2: ติดตั้งแบบแมนนวล (Manual)
1. ดาวน์โหลดไฟล์คอมไพล์สำเร็จ `air-quality-card.js` จากโฟลเดอร์ `dist/` ในโปรเจกต์นี้
2. นำไฟล์ดังกล่าวไปอัปโหลดไว้ในเซิร์ฟเวอร์ Home Assistant ของคุณภายใต้โฟลเดอร์ `config/www/` (แนะนำให้สร้างโฟลเดอร์ย่อย เช่น `config/www/air-quality-card/dist/air-quality-card.js`)
3. ดาวน์โหลดโฟลเดอร์ `img/` ไปวางไว้ในพาธเดียวกันเพื่อให้การ์ดเรียกใช้รูปภาพเรตติงได้
4. ทำการลงทะเบียน Resource ในแดชบอร์ดระบบเพื่อให้ Home Assistant รู้จักการ์ด:
   - ไปที่ **การตั้งค่า (Settings)** → **แดชบอร์ด (Dashboards)** → คลิกจุด 3 จุดมุมขวาบน เลือก **ทรัพยากร (Resources)**
   - กดปุ่ม **เพิ่มทรัพยากร (Add Resource)** แล้วกรอกข้อมูลดังนี้:
     - **URL:** `/local/air-quality-card/dist/air-quality-card.js`
     - **ประเภททรัพยากร (Resource Type):** `JavaScript Module`
...
รูปภาพสัญลักษณ์ (Badge) ที่มุมซ้ายบนของการ์ด จะเปลี่ยนกราฟิกตามคีย์เวิร์ดเฉพาะเท่านั้น Entity ในช่อง rating ของคุณ จำเป็นต้องส่งค่า ออกมาเป็นคำเหล่านี้คำใดคำหนึ่ง:
excellent, good, moderate, poor, unhealthy

หากระบบเซ็นเซอร์คุณภาพอากาศที่คุณใช้อยู่ (เช่น AirVisual, WAQI) ส่งค่าออกมาเป็นตัวเลข AQI หรือเป็นคำศัพท์อื่นๆ คุณจะต้องสร้าง Template Sensor ในไฟล์ configuration.yaml เพื่อแปลงค่าเหล่านั้นให้ตรงกับที่การ์ดต้องการ
